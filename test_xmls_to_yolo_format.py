...
dest_ann_folder = "annotations_test"
...
def annote():
    # Get the annotations
    annotations = [os.path.join('test_xmls', x) for x in os.listdir('test_xmls') if x[-3:] == "xml"]
    annotations.sort()
...
def plot_random_image_with_bbox(annotations):
    ...
    image_file = annotation_file.replace("annotations_test", "Test").replace("txt", "png")
    print(image_file)
