...
save_root2 = "train_xmls"

print("Conversion from CSV to XML for the Train.csv file ...")

if not os.path.exists(save_root2):
    os.mkdir(save_root2)

def filename_to_str(f):
     # Change the path to avoid using "/" in file name and delete unneccessary folder name
...
def write_xml(folder, filename, bbox_list):
    ...
    xml_filename = os.path.join('.', folder, os.path.splitext('Train_' + filename)[0] + '.xml')
